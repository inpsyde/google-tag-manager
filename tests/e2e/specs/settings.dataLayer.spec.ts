...
test.describe( 'Plugin Settings - DataLayer', () => {
	...
	test(
		'I can set successfully a GTM ID.',
		async ( {
			pluginSettingsPage,
		} ) => {
			...
			await expect( pluginSettingsPage.successMessage() )
				.toBeVisible();
			await expect( pluginSettingsPage.gtmIdInput() )
				.toHaveValue( 'GTM-12345' );
		}
	);

	test(
		'I can set successfully a dataLayer name.',
		async ( {
			pluginSettingsPage,
		} ) => {
			...
			await expect( pluginSettingsPage.successMessage() )
				.toBeVisible();
			await expect( pluginSettingsPage.dataLayerNameInput() )
				.toHaveValue( 'myDataLayer' );
		}
	);

	test(
		'I can enable and disable the noscript.',
		async ( {
			pluginSettingsPage,
		} ) => {
			...
			await expect( pluginSettingsPage.successMessage() )
				.toBeVisible();
			await expect( pluginSettingsPage.autoInsertNoscriptSelect() )
				.toHaveValue( 'enable' );
	...
	test(
		'I can enable or disable collectors.',
		async ( {
			pluginSettingsPage,
		} ) => {
			...
			//@ts-ignore
			for (let [collector, selected] of collectors) {
				selected
			...
			//@ts-ignore
			for (let [collector, selected] of collectors) {
				const check = expect( pluginSettingsPage.enableCollectorCheckbox( collector ) );
				selected
